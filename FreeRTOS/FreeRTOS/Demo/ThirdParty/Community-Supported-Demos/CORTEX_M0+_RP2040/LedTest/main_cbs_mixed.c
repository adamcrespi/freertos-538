 ...
 *
 * Two periodic EDF tasks + two independent CBS servers.
 * Demonstrates multiple CBS servers coexisting with periodic tasks,
 * each maintaining its own budget and deadline independently.
 *
 * Task Set:
 *   τ1 (Red):     Periodic  C=50ms   D=200ms  T=400ms   U=0.125
 *   τ2 (Yellow):  Periodic  C=100ms  D=400ms  T=800ms   U=0.125
 *   S1 (Green):   CBS       Qs=80ms  Ts=400ms            Us=0.20
 *   S2 (Blue):    CBS       Qs=100ms Ts=500ms            Us=0.20
 *   Total U = 0.65 (schedulable)
 *
...
static PeriodicParams_t xTask2Params = { YELLOW_PIN, TASK2_WCET, TASK2_PERIOD, "Yellow" };
static CBSParams_t      xCBS1Params  = { GREEN_PIN,  CBS1_BUDGET, CBS1_PERIOD, "Green/S1" };
static CBSParams_t      xCBS2Params  = { BLUE_PIN,   CBS2_BUDGET, CBS2_PERIOD, "Blue/S2" };
...
int main( void )
{
    ...
    xResult = xTaskCreateEDF( vPeriodicTask, "Yellow", 512, &xTask2Params,
                              TASK2_PERIOD, TASK2_DEADLINE, TASK2_WCET, NULL );
    printf( "Create Yellow (periodic): %s\n", xResult == pdPASS ? "OK" : "FAIL" );

    /* Create CBS servers */
    xResult = xTaskCreateCBS( vCBSTask, "GreenCBS", 512, &xCBS1Params,
                              CBS1_BUDGET, CBS1_PERIOD, NULL );
    printf( "Create Green/S1 (CBS):    %s\n", xResult == pdPASS ? "OK" : "FAIL" );

    xResult = xTaskCreateCBS( vCBSTask, "BlueCBS", 512, &xCBS2Params,
                              CBS2_BUDGET, CBS2_PERIOD, NULL );
    printf( "Create Blue/S2 (CBS):     %s\n", xResult == pdPASS ? "OK" : "FAIL" );
